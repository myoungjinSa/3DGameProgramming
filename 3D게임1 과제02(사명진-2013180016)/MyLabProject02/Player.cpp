...
CPlayer::CPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	pBulletMesh = new CBulletMeshDiffused(pd3dDevice,pd3dCommandList,3.0f,3.0f,10.0f);			//x=1.0,y=1.0,z=5.0
	...
	m_sphereBox = new BoundingSphere(XMFLOAT3(0.0f, 0.0f, 0.0f), 50.0f);

	//this->SetPlayerUpdatedContext(pColor);
...
CPlayer::~CPlayer()
{
	...
	if (this->m_pBulletObjects) {
		for (int i = 0; i < CPlayer::m_countBullet; i++) {
			delete m_pBulletObjects[i];
			m_pBulletObjects[i] = NULL;
		}
		delete m_pBulletObjects;
		m_pBulletObjects = NULL;
...
void CPlayer::Rotate(float x, float y, float z)
{
	...
	//	this->m_pBulletObjects[i]->RotateBullet(m_fPitch, m_fYaw, m_fRoll);
	//}
	//		
}
...
void CPlayer::focus_On(CGameObject *pSelectedObject)
{
	LookAt(pSelectedObject->GetPosition(), m_xmf3Up);
}
...
CAirplanePlayer::CAirplanePlayer(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
	*pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature):CPlayer(pd3dDevice,pd3dCommandList)
{
	//비행기 메쉬를 생성한다.
