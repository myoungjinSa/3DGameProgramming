...
public:
	...
	XMFLOAT4 GetDiffused() {
		return m_xmf4Diffuse;
	}

	void SetDiffused(XMFLOAT4 xmDiffuse) {
		m_xmf4Diffuse = xmDiffuse;
	}
};
...
public:
	...
	CDiffusedVertex* m_DiffuseVertex;
protected:
...
class CBulletMeshDiffused : public CMesh
{
public:
	//직육면체의 가로, 세로, 깊이의 길이를 지정하여 직육면체 메쉬를 생성한다.
	CBulletMeshDiffused(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList
		*pd3dCommandList, float fWidth = 2.0f, float fHeight = 2.0f, float fDepth = 2.0f);
