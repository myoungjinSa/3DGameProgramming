...
//
//	//이미지상의 지형 정점
//	m_pHeightMapColor = new XMFLOAT4[m_nWidth * m_nLength];				//하이트맵 이미지의 픽셀 수
//	for (int y = 0; y < m_nLength; y++)
//	{
//		for (int x = 0; x < m_nWidth; x++)
//		{
...
//	m_nStride = sizeof(CTexturedVertex);
	...
	for (int i = 0,z = zStart; z < (zStart + nLength); z++)
	{
		for (int x = xStart; x < (xStart + nWidth); x++, i++)
		{
			...
			//파티션마다 색상 부여, 즉 for문이 한번 돌면 다른 색상 다른 구역이 된다. 
			...
			//if ((UINT)arr[i]==49)/*arr == 1*/	//1은 나무가 세워질 공간
			//{

			//	m_positionVector.emplace_back(pVertices[i].m_xmf3Position);			//위치값을 벡터의 저장 
			//}
