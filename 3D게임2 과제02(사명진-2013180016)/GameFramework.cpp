...
void CGameFramework::OnProcessingKeyboardMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_F3:
			...
			break;
		/*case VK_SPACE:
			m_bFillmode = !m_bFillmode;
			break;*/
		case VK_SHIFT:
			m_pScene->keyCallCount++;
			if (CScene::keyCallCount > 3)
			{
				CScene::keyCallCount = 0;
			}
			break;
		case VK_F9:
		{
...
void CGameFramework::BuildObjects()
{
	...
	pAirplanePlayer->SetPosition(XMFLOAT3(1000.0f, 300.0f, 500.0f));
	...
	if (!bProcessedByScene)
	{
		...
		if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
		{
			...
			if (dwDirection)
			{
				...
				m_pPlayer->Rotate(dwDirection*30.0f, 0.0f, 0.0f);
			}
		}
	}
...
void CGameFramework::FrameAdvance()
{
	...
	d3dResourceBarrier.Transition.StateBefore = D3D12_RESOURCE_STATE_PRESENT;		// 후면 버퍼가 프리젠트 될때 상태
	d3dResourceBarrier.Transition.StateAfter = D3D12_RESOURCE_STATE_RENDER_TARGET;  // 렌더 타겟을 사용하기 위한 상태
	...
	m_pScene->Render(m_pd3dCommandList,m_bFillmode, m_pCamera);

#ifdef _WITH_PLAYER_TOP
	m_pd3dCommandList->ClearDepthStencilView(d3dDsvCPUDescriptorHandle, D3D12_CLEAR_FLAG_DEPTH | D3D12_CLEAR_FLAG_STENCIL, 1.0f, 0, 0, NULL);
#endif
	m_pPlayer->Render(m_pd3dCommandList,m_bFillmode ,m_pCamera);
