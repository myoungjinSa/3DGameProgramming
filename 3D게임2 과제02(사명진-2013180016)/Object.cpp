...
void CGameObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera, UINT uInstance,int objNum,UINT rasterNum)
{
...
void CGameObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera,int objNum,UINT rasterNum)
{
	if (IsVisible(objNum, pCamera)) {
		OnPrepareRender();
...
void CGameObject::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera, UINT rasterNum)
{
...
void CHeightMapTerrain::UpdateShaderVariables(ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	/*for (int i = 0; i <= m_partsID; i++) {
		m_pcbMappedGameObject->m_PartsID[i] = i;
	}*/
...
CHeightMapTerrain::CHeightMapTerrain(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, ID3D12RootSignature *pd3dGraphicsRootSignature, LPCTSTR pFileName,int nWidth, int nLength, int nBlockWidth, int nBlockLength, XMFLOAT3 xmf3Scale, XMFLOAT4 xmf4Color,UINT id) : CGameObject(0)
{
	...
	//m_pHeightColorMapImage = new CHeightColorImage(pFileName2, nWidth, nLength, xmf3Scale);		//노멀맵 이미지 생성 ,같은 크기
	...
	//std::vector<XMFLOAT3>::iterator iter;
	for (int z = 0, zStart = 0; z < czBlocks; z++)
	{
		for (int x = 0, xStart = 0; x < cxBlocks; x++)
		{
			xStart = x * xStart;									//xStart == 월드 좌표계 시작점
			zStart = z * zStart;								//zStart == 월드 좌표계 시작점
			pHeightMapGridMesh = new CHeightMapGridMesh(pd3dDevice, pd3dCommandList, xStart, zStart, nBlockWidth, nBlockLength, xmf3Scale, xmf4Color, m_pHeightMapImage);
			//pHeightMapGridMesh->OnGetHeight()
			
			SetMesh(x + (z*cxBlocks), pHeightMapGridMesh);
		/*	for (iter = pHeightMapGridMesh->m_positionVector.begin();iter!=pHeightMapGridMesh->m_positionVector.end();++iter) 
			{
				SetTreePosition(index++, *iter);
			}*/
		}
	}
	...
	//CTexture *pTerrainTexture[5];
	//pTerrainTexture[0] = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	//pTerrainTexture[0]->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Base_Texture.dds", 0);
	//
	//pTerrainTexture[1] = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	//pTerrainTexture[1]->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Detail_Texture_7.dds", 1);
	//
	//pTerrainTexture[2] = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	//pTerrainTexture[2]->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Texture/Sand01S.dds", 2);
	//
	//pTerrainTexture[3] = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	//pTerrainTexture[3]->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Texture/HeightMap.dds", 3);
	//
	//pTerrainTexture[4] = new CTexture(1, RESOURCE_TEXTURE2D, 0);
	//pTerrainTexture[4]->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Texture/HeightPlaneMap.dds", 4);
	...
	//pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Texture/HeightMap.dds", 3);
	//pTerrainTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Texture/HeightPlaneMap.dds", 4);

	UINT ncbElementBytes = ((sizeof(CB_GAMEOBJECT_INFO) + 255) & ~255); //256의 배수
	//UINT ncbPartitionBytes = ((sizeof(CB_TERRAIN_PARTS) + 255) & ~255);
	
	CTerrainShader *pTerrainShader = new CTerrainShader();
	pTerrainShader->CreateShader(pd3dDevice, pd3dGraphicsRootSignature);
	pTerrainShader->CreateShaderVariables(pd3dDevice, pd3dCommandList,m_nMeshes);
	...
	//pTerrainShader->CreateConstantBufferViews(pd3dDevice, pd3dCommandList, 1, pTerrainShader->m_pd3dcbTerrainParts, ncbPartitionBytes);
...
void CHeightMapTerrain::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera, UINT rasterNum) {
	m_pMaterial->m_pShader->OnPrepareRender(pd3dCommandList, rasterNum);
	//m_pMaterial->m_pShader->Render(pd3dCommandList, rasterNum, pCamera);
	if (m_pMaterial)
	{
		if (m_pMaterial->m_pShader)
...
void CHeightMapTerrain::SetTreePosition(int index,XMFLOAT3 &v)
{

	vecTreePosition.push_back(v);
...
CHeightMapTerrain::~CHeightMapTerrain(void)
{
	...
	//if (m_pHeightColorMapImage) delete m_pHeightColorMapImage;
}
//////////////////////////////////////////////////////////////
//
...
void CSkyBox::Render(ID3D12GraphicsCommandList *pd3dCommandList,bool bFillmode ,CCamera *pCamera)
{
	...
	if (m_pMaterial)
	{
		if (m_pMaterial->m_pShader)
		{
			m_pMaterial->m_pShader->Render(pd3dCommandList,bFillmode, pCamera);
			m_pMaterial->m_pShader->UpdateShaderVariables(pd3dCommandList);
