...
void CGameFramework::BuildFrameBuffer()
{
	...
	m_hBitmapFrameBuffer = ::CreateCompatibleBitmap(hDC, (m_rcClient.right -m_rcClient.left) + 1, (m_rcClient.bottom - m_rcClient.top) + 1);
...
void CGameFramework::OnProcessingMouseMessage(HWND hWnd, UINT nMessageID, WPARAM wParam, LPARAM lParam)
{
	switch (nMessageID)
	{
		...
		case WM_RBUTTONDOWN:
			...
			m_pScene->OnMouseDown(wParam, GET_X_LPARAM(lParam),GET_Y_LPARAM(lParam));
			break;
		case WM_LBUTTONUP:
		case WM_RBUTTONUP:
			::ReleaseCapture();
...
void CGameFramework::SetClientRect()
{
	m_pScene->SetClientRect(CLIENT_WIDTH, CLIENT_HEIGHT);
	
}

void CGameFramework::BuildObjects(HINSTANCE hInstance)
{
	...
	m_pScene = new CScene();
	m_pScene->BuildObjects(hInstance ,m_pPlayer);
...
{    
	...
	//cout <<"plane:" <<'(' << m_pPlayer->m_xmf3Look.x << ',' << m_pPlayer->m_xmf3Look.y << ',' << m_pPlayer->m_xmf3Look.z << ')' << endl;
	PresentFrameBuffer();
...
void CGameFramework::GeneratePickingRay()
{
