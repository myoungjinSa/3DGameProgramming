...
void CObjectsShader::CreateShaderVariables(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList)
{
	...
	m_pd3dcbGameObjects->Map(0, NULL, (void **)&m_pcbMappedGameObjects);	//m_pcbMappedGameObjects 는 영구적으로 cpu가 접근 가능한 주소가 된다. 이 리소스에 대한 포인터는 영구적으로 위치를 안바꾼다.
...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList, void *pContext)
{
	...
	float fxPitch = 12.0f * 3.5f;					//이 값이 커질수록 오브젝트의 수가 적어진다.
...
//	pTexture->LoadTextureFromFile(pd3dDevice, pd3dCommandList, L"Image/Lava(Emissive).dds", 0);
	...
	CreateConstantBufferViews(pd3dDevice, pd3dCommandList, m_nObjects, m_pd3dcbGameObjects, ncbElementBytes); //오브젝트를 상수 버퍼뷰 생성
	CreateShaderResourceViews(pd3dDevice, pd3dCommandList, pTexture, 3, false);				//SRV 버퍼 뷰 생성
